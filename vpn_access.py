...
class Vpn_Access_Main:
    ...
    def run(self):
        ...
        # if self.vpn_type in ['VESSEL', 'RH', 'CLIENT', 'RHADMIN', 'GBF']:
        if self.vpn_type in vpn_type_options:
            ...
            if status == 'ok':
                if self.vpn_request_type == 'CREATE':
                    ...
                    if status == 'ok' and self.vpn_type in ['CLIENT', 'GBF', 'RH', 'RHADMIN']:

                        self.postgresql_query.connection()
                        # GET VPN IPS
                        sql_str = "SELECT ip_addr_1, ip_addr_2  FROM account_vpn_access WHERE id_vpn_access_requests = '{}' AND vpn_type = '{}'".format(id_vpn_access_requests, self.vpn_type)
                        print("*"*100)
                    ...
                    # ALLOW/DISALLOW FOR USER-VPN (API AND WEB)
                    if self.vpn_type in ['CLIENT', 'GBF', 'RH', 'RHADMIN']:
                        self.postgresql_query.connection()
...
def vpn_terminal_receiver():
    ...
    if vpn_type in ['CLIENT', 'GBF', 'RH', 'RHADMIN']:
